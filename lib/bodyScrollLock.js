(function (global, factory) {
  ...
  function findScrollableElement(event) {
    var path = event.composePath ? event.composePath() : composedPath(event.target);
    return path.find(isScrollable);
  }

  var handleScroll = function handleScroll(event) {
    var clientY = event.targetTouches[0].clientY - initialClientY;
    var scrollableElement = findScrollableElement(event);
